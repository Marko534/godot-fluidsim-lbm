// Main Buffers and Parameters
// #[compute]
// #version 460 


layout (std430, binding = 0) buffer f {
  float F[];
};

layout (std430, binding = 1) buffer fprop {
  float FPROP[];
};

layout (std430, binding = 2) buffer b {
  float B[];
};

layout(set = 0, binding = 3, rgba32f) uniform restrict image3D velocity_tex_3d;

layout(std140, binding = 5) uniform SimParams {
    int NX;
    int NY;
    int NZ;
    float t;
} params;


// vec3 force = vec3(0, -0.1, 0);

vec3 force = vec3(0.1, 0, 0);

// vec3 force = 0.1*vec3(cos(params.t), 0, sin(params.t));

// vec3 force = 0.1*vec3(1, 0, 0);

//vec3(cos(t), 0, sin(t));

// vec3 force = 0.1*vec3(cos(t), 0, sin(t));

const int Q = 15;

const float w[Q] = {
    2.0/9.0,                              // rest
    1.0/9.0, 1.0/9.0, 1.0/9.0,           // ±x, ±y, ±z faces
    1.0/9.0, 1.0/9.0, 1.0/9.0,
    1.0/72.0, 1.0/72.0, 1.0/72.0,        // body-diagonal corners
    1.0/72.0, 1.0/72.0, 1.0/72.0,
    1.0/72.0, 1.0/72.0
};

const ivec3 c[Q] = {
    ivec3( 0,  0,  0),                    // 0: rest
    ivec3( 1,  0,  0), ivec3(-1,  0,  0), // 1,2: ±x
    ivec3( 0,  1,  0), ivec3( 0, -1,  0), // 3,4: ±y
    ivec3( 0,  0,  1), ivec3( 0,  0, -1), // 5,6: ±z
    ivec3( 1,  1,  1), ivec3(-1, -1, -1), // 7,8: corners
    ivec3( 1,  1, -1), ivec3(-1, -1,  1), // 9,10
    ivec3( 1, -1,  1), ivec3(-1,  1, -1), // 11,12
    ivec3(-1,  1,  1), ivec3( 1, -1, -1)  // 13,14
};

const int cp[Q] = {
    0,
    2, 1, 4, 3, 6, 5,   // face opposites
    8, 7, 10, 9, 12, 11, 14, 13  // corner opposites
};

const float cs = 1.0/sqrt(3.0);
const float cs2 = 1.0/cs/cs;
const float cs4 = 1.0/cs/cs/cs/cs;

// Compute the Equilibrium Boltzmann Distribution

float equilibrium(int q, float rho, vec3 v){

  float eq = 0.0;
  eq += w[q]*rho;
  eq += w[q]*rho*(dot(v, c[q]))*cs2;
  eq += w[q]*rho*(dot(v, c[q])*dot(v, c[q]))*0.5*cs4;
  eq -= w[q]*rho*(dot(v, v))*0.5*cs2;
  return eq;

}

// Compute the Density at a Position

float getRho(uint index){

  float rho = 0.0;
  for(int q = 0; q < Q; q++)
    rho += F[index*Q + q];
  return rho;

}

// Compute the Momentum at a Position

vec3 getV(uint index){

  vec3 v = vec3(0);
  for(int q = 0; q < Q; q++)
    v += F[index*Q + q]*c[q];
  return v;

}

// Replace getRho and getV with this single function
void getRhoV(uint index, out float rho, out vec3 v, out float f_cache[Q]){
    rho = 0.0;
    v   = vec3(0.0);
    for(int q = 0; q < Q; q++){
        float f    = F[index*Q + q];
        f_cache[q] = f;
        rho        += f;
        v          += f * vec3(c[q]);
    }
    v /= rho;
}
